...
class Reflector : public sigc::trackable
{
  public:
    ...
    // jayReflector: called by ReflectorClient and TrunkLink
    void sendNodeListToAllPeers(void);
    void onPeerNodeList(TrunkLink* link,
                        const std::vector<MsgTrunkNodeList::NodeEntry>& nodes);
    void scheduleNodeListUpdate(void);  // debounced sendNodeListToAllPeers
    // jayReflector: called by SatelliteLink on talker events
    void onSatelliteTalker(SatelliteLink* link, uint32_t tg,
                           const std::string& callsign, bool active);
    // jayReflector: called by SatelliteLink when satellite sends its node list
    void onSatelliteNodeList(SatelliteLink* link,
        const std::vector<MsgTrunkNodeList::NodeEntry>& nodes);
  ...
  private:
    ...
    // jayReflector extensions
    void handleTrunkPtyCmd(const std::string& cmd);
    void publishTrunkStatusMqtt(void);
    void publishPeerNodesMqtt(TrunkLink* link);
    void publishLocalNodesMqtt(const std::vector<MsgTrunkNodeList::NodeEntry>& nodes);
    void publishSatelliteTalkerMqtt(SatelliteLink* link, uint32_t tg,
                                    const std::string& callsign, bool active);
    void publishSatelliteStatusMqtt(SatelliteLink* link);
    void publishSatelliteNodesMqtt(SatelliteLink* link,
        const std::vector<MsgTrunkNodeList::NodeEntry>& nodes);
    void publishTalkerMqtt(uint32_t tg, const std::string& callsign,
        bool active);
    void sendMqtt(const std::string& topic,
        const std::string& payload, bool retain);
    std::string         m_mqtt_host;
    int                 m_mqtt_port = 1883;
    std::string         m_mqtt_topic_prefix;
    bool                m_mqtt_trunk_enabled = false;
    bool                m_mqtt_talker_enabled = false;
    std::string         m_last_local_nodes_payload;
    int                 m_mqtt_sock_fd = -1;
    std::string         m_mqtt_sock_path;
    Async::Timer        m_nodelist_timer;   // debounce for sendNodeListToAllPeers
