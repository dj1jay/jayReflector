...
SatelliteClient::SatelliteClient(Reflector* reflector, Async::Config& cfg)
  : m_reflector(reflector), m_cfg(cfg),
    ...
    m_hb_tx_cnt(0), m_hb_rx_cnt(0), m_nodelist_keepalive_cnt(0)
{
...
bool SatelliteClient::initialize(void)
{
  ...
  // SATELLITE_FILTER: TGs this satellite wants from parent
  // Syntax: exact, prefix (24*), range (2427-2438), comma-separated
  m_cfg.getValue("GLOBAL", "SATELLITE_FILTER", m_tg_filter_str);
  if (!m_tg_filter_str.empty())
  {
    m_tg_filter = TgFilter::parse(m_tg_filter_str);
    cout << "SATELLITE: TG filter (bidirectional): "
         << m_tg_filter.toString() << endl;
  }
                                          ...
                                          const std::string& callsign)
{
  ...
  if (!m_tg_filter.matches(tg)) return;  // bidirectional filter
...
{
  if (!m_con.isConnected() || !m_hello_received) return;
  if (!m_tg_filter.matches(tg)) return;
...
void SatelliteClient::onConnected(void)
{
  ...
  m_nodelist_keepalive_cnt = NODELIST_KEEPALIVE_CNT;
...
void SatelliteClient::handleMsgTrunkHello(std::istream& is)
{
  ...
  // Send TG filter to parent if configured (MsgTrunkFilter type 122)
  // Old GeuReflector parents silently ignore unknown message types
  if (!m_tg_filter_str.empty())
  {
    sendMsg(MsgTrunkFilter(m_tg_filter_str));
    cout << "SATELLITE: Sent TG filter: " << m_tg_filter_str << endl;
  }
...
void SatelliteClient::heartbeatTick(Async::Timer* t)
{
  ...
  // Periodic node list keepalive — sends current node list to parent
  if (--m_nodelist_keepalive_cnt == 0)
  {
    m_nodelist_keepalive_cnt = NODELIST_KEEPALIVE_CNT;
...
{
  if (!m_con.isConnected() || !m_hello_received) return;
  cout << "SATELLITE: sending node list (" << nodes.size()
       << " nodes) to parent" << endl;
