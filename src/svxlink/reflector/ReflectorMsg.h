...
/**
@brief  Trunk node list message (jayReflector extension)
@date   2026-04-06

Sent by a reflector to its trunk peer whenever its local client list changes
(login, logout, TG change). The peer stores the list and publishes it via MQTT.
Unknown message types are silently ignored by GeuReflector — fully backward
compatible.

Each entry is: callsign (string) + current_tg (uint32)
*/
...
/**
 * @brief  Trunk filter message — sent after Hello to advertise TG filter
 *
 * Optional message sent by satellites after Hello to tell the parent
 * which TGs they want to receive. Old GeuReflector peers silently ignore
 * unknown message types — fully backward compatible.
 *
 * Type 122 — jayReflector extension
 */
