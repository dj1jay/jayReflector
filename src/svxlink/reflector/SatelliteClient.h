...
class SatelliteClient : public sigc::trackable
{
  public:
    ...
    // Called by Reflector when local node list changes (login/logout/TG change)
  ...
  private:
    ...
    static const unsigned NODELIST_KEEPALIVE_CNT  = 60;  // send node list every 60s
    ...
    unsigned        m_nodelist_keepalive_cnt;
    std::string     m_tg_filter_str;  // sent to parent in Hello
    TgFilter        m_tg_filter;       // parsed filter for bidirectional blocking
